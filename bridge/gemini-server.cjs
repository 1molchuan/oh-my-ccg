...
const SERVER_INFO = { name: 'oh-my-ccg-gemini', version: '1.0.0' };
const jobs = new Map();

const TOOLS = [
  {
    name: 'ask_gemini',
    description: 'Send a prompt to Gemini (gemini-3-pro-preview) for analysis. Recommended roles: designer, writer, vision. 1M token context window.',
    inputSchema: {
      type: 'object',
      properties: {
        agent_role: { type: 'string', description: 'Agent perspective for Gemini' },
...
function runGemini(args) {
  ...
  try {
    const parts = ['codeagent-wrapper', '--backend', 'gemini'];
